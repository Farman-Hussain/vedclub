...
import DoctorSidebar from "@/app/components/DoctorSidebar"; // 🔥 Imported Popup!

async function getTreatment(slug: string) {
  const query = `*[_type == "treatment" && slug.current == $slug][0]{ ..., author-> }`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!treatment) return {};
  return { title: treatment.seoTitle || `${treatment.title} | Ved Club`, description: treatment.seoDescription || treatment.shortDescription };
}
      ...
      <section className="bg-[#1A361A] text-white pt-12 pb-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 animate-fade-in">
            <Link href="/treatments" className="inline-flex items-center text-green-200 hover:text-white mb-8 transition-colors text-sm font-medium">
              <ArrowLeft size={16} className="mr-2" /> Back to Procedures
            </Link>
            <div className="flex items-center gap-3 mb-4">
              <span className="bg-green-800 text-green-100 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Ayurvedic Therapy</span>
          ...
          <div className="sticky top-28 space-y-6">
            <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100">
              ...
              <ul className="space-y-3 text-gray-600 font-medium text-sm">
                {treatment.overview && <li><a href="#overview" className="hover:text-[#1EAD16] transition">Overview</a></li>}
                {treatment.benefits && <li><a href="#benefits" className="hover:text-[#1EAD16] transition">Benefits</a></li>}
                {treatment.steps && <li><a href="#steps" className="hover:text-[#1EAD16] transition">Procedure Steps</a></li>}
                {treatment.whoIsItFor && <li><a href="#indications" className="hover:text-[#1EAD16] transition">Best Suited For</a></li>}
                {treatment.additionalContent && <li><a href="#additional" className="hover:text-[#1EAD16] transition">{treatment.additionalContentHeading || 'Additional Info'}</a></li>}
                {treatment.summary && <li><a href="#summary" className="hover:text-[#1EAD16] transition">Summary</a></li>}
                {treatment.faqs && <li><a href="#faqs" className="hover:text-[#1EAD16] transition">FAQs</a></li>}
                {treatment.references && <li><a href="#references" className="hover:text-[#1EAD16] transition">References</a></li>}
              </ul>
            </div>

            {/* 🔥 NEW INTERACTIVE POPUP SIDEBAR */}
        ...
        <div className="md:w-3/4 space-y-12">
          {treatment.overview && (
            <section id="overview" className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100 scroll-mt-28">
              <h2 className="text-2xl font-bold text-[#1A361A] mb-6 flex items-center gap-2"><Activity className="text-[#1EAD16]" /> What is {treatment.title}?</h2>
          ...
          {treatment.benefits && (
            <section id="benefits" className="bg-green-50/50 p-8 md:p-10 rounded-3xl border border-green-100 scroll-mt-28">
              <h2 className="text-2xl font-bold text-[#1A361A] mb-6 flex items-center gap-2"><CheckCircle2 className="text-[#1EAD16]" /> Key Health Benefits</h2>
          ...
          {treatment.steps && (
            <section id="steps" className="scroll-mt-28">
              <h2 className="text-3xl font-bold text-[#1A361A] mb-8">How it Works (Procedure Steps)</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {treatment.whoIsItFor && (
              <section id="indications" className="bg-blue-50/50 p-8 rounded-3xl border border-blue-100 scroll-mt-28">
                <h3 className="text-xl font-bold text-[#1A361A] mb-4 flex items-center gap-2"><HeartPulse className="text-blue-500" /> Best Suited For</h3>
            ...
            {treatment.contraindications && (
              <section className="bg-red-50/50 p-8 rounded-3xl border border-red-100">
                <h3 className="text-xl font-bold text-red-800 mb-4 flex items-center gap-2"><AlertTriangle className="text-red-500" /> Avoid If (Contraindications)</h3>
          ...
          {treatment.additionalContent && (
            <section id="additional" className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm scroll-mt-28">
              <h2 className="text-2xl font-bold text-[#1A361A] mb-6">{treatment.additionalContentHeading || 'Additional Information'}</h2>
          ...
          {treatment.summary && (
            <section id="summary" className="bg-gradient-to-br from-green-50 to-white p-8 rounded-3xl border border-green-100 shadow-sm scroll-mt-28">
              <h2 className="text-2xl font-bold text-[#1A361A] mb-4">Conclusion</h2>
          ...
          {treatment.author && (
            <section className="bg-white p-8 md:p-10 rounded-3xl shadow-lg border border-green-100 mt-16 hover:-translate-y-1 transition duration-300">
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Medical Reviewer</h3>
