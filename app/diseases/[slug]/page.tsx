...
async function getDisease(slug: string) {
  const query = `*[_type == "disease" && slug.current == $slug][0]{ ..., author-> }`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!data) return {};
  return { title: data.seoTitle || `${data.title} Ayurvedic Treatment | Ved Club`, description: data.seoDescription };
}
      ...
      <section className="bg-red-50 pt-12 pb-24 px-4 sm:px-6 lg:px-8 border-b border-red-100">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 animate-fade-in">
            <Link href="/diseases" className="inline-flex items-center text-gray-500 hover:text-red-600 mb-8 transition-colors text-sm font-medium">
              <ArrowLeft size={16} className="mr-2" /> Back to Diseases
            </Link>
            <div className="flex items-center gap-3 mb-4">
              <span className="bg-red-200 text-red-800 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 shadow-sm"><HeartPulse size={14}/> Disease Management</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 flex flex-col md:flex-row gap-12 relative z-10">
        ...
        <div className="md:w-1/4 hidden md:block">
          <div className="sticky top-28 space-y-6">
            <div className="bg-white p-6 rounded-3xl shadow-xl border border-gray-100 transition-all hover:shadow-2xl">
              <h3 className="font-bold text-[#1A361A] mb-4 uppercase tracking-wider text-sm flex items-center gap-2"><BookOpen size={16} className="text-red-500"/> Contents</h3>
              <ul className="space-y-3 text-gray-600 font-medium text-sm">
                {disease.overview && <li><a href="#overview" className="hover:text-red-500 transition-colors">Modern View</a></li>}
                {disease.ayurvedicPerspective && <li><a href="#ayurveda" className="hover:text-red-500 transition-colors font-bold text-[#1A361A]">Ayurvedic Perspective</a></li>}
                {disease.treatments && <li><a href="#treatment" className="hover:text-red-500 transition-colors">Herbs & Treatment</a></li>}
                {disease.homeRemedies && <li><a href="#remedies" className="hover:text-red-500 transition-colors">Home Remedies</a></li>}
                {disease.dietLifestyle && <li><a href="#lifestyle" className="hover:text-red-500 transition-colors">Diet & Lifestyle</a></li>}
                
                {/* DYNAMIC ADDITIONAL SECTION LINK */}
                {disease.additionalSectionContent && <li><a href="#additional" className="hover:text-red-500 transition-colors">{disease.additionalSectionHeading || "Additional Information"}</a></li>}
                
                {disease.faqs && disease.faqs.length > 0 && <li><a href="#faqs" className="hover:text-red-500 transition-colors">FAQs</a></li>}
                {disease.conclusion && <li><a href="#conclusion" className="hover:text-red-500 transition-colors">Conclusion</a></li>}
              </ul>
            </div>
        ...
        <div className="md:w-3/4 space-y-12">
          ...
          {disease.overview && (
            <article id="overview" className="scroll-mt-28 bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              <h2 className="text-2xl font-bold text-[#1A361A] mb-6 flex items-center gap-2"><Search className="text-gray-400"/> Overview & Modern Science</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {disease.symptoms && (
              <div className="bg-yellow-50 p-8 rounded-3xl border border-yellow-100 hover:-translate-y-1 transition-transform duration-300 hover:shadow-lg">
                <h3 className="text-xl font-bold text-yellow-900 mb-4 flex items-center gap-2"><ShieldAlert className="text-yellow-600" /> Common Symptoms</h3>
            ...
            {disease.causes && (
              <div className="bg-gray-50 p-8 rounded-3xl border border-gray-200 hover:-translate-y-1 transition-transform duration-300 hover:shadow-lg">
                <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Activity className="text-gray-500" /> Causes & Triggers</h3>
          ...
          {disease.ayurvedicPerspective && (
            <section id="ayurveda" className="bg-[#1A361A] text-white p-8 md:p-12 rounded-[3rem] shadow-2xl scroll-mt-28 relative overflow-hidden group">
              ...
              <h2 className="text-3xl font-extrabold mb-6 text-green-100 flex items-center gap-3 relative z-10"><Leaf className="text-[#1EAD16]"/> The Ayurvedic Perspective</h2>
          ...
          {disease.treatments && (
            <section id="treatment" className="scroll-mt-28 bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              <h2 className="text-3xl font-bold text-[#1A361A] mb-6 flex items-center gap-2"><Pill className="text-blue-500"/> Recommended Herbs & Treatment</h2>
        ...
        {disease.homeRemedies && (
              <section id="remedies" className="scroll-mt-28 bg-green-50 p-8 rounded-3xl border border-green-100 hover:-translate-y-1 transition-transform duration-300 hover:shadow-lg">
                <h2 className="text-2xl font-bold text-green-900 mb-6 flex items-center gap-2"><Droplets className="text-[#1EAD16]"/> Home Remedies</h2>
            ...
            {disease.dietLifestyle && (
              <section id="lifestyle" className="scroll-mt-28 bg-orange-50 p-8 rounded-3xl border border-orange-100 hover:-translate-y-1 transition-transform duration-300 hover:shadow-lg">
                <h2 className="text-2xl font-bold text-orange-900 mb-6 flex items-center gap-2"><Utensils className="text-orange-500"/> Diet & Lifestyle (Ahara-Vihara)</h2>
          ...
          {disease.additionalSectionContent && (
            <section id="additional" className="scroll-mt-28 bg-blue-50 p-8 md:p-10 rounded-[2.5rem] border border-blue-100 shadow-sm hover:shadow-md transition-shadow">
              <h2 className="text-3xl font-bold text-[#1A361A] mb-6 flex items-center gap-2">
                <Star className="text-blue-500"/> {disease.additionalSectionHeading || "Additional Information"}
              </h2>
          ...
          {disease.faqs && disease.faqs.length > 0 && (
            <section id="faqs" className="scroll-mt-28 bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100">
              <h2 className="text-3xl font-bold text-[#1A361A] mb-8 flex items-center gap-2"><HelpCircle className="text-red-500"/> Frequently Asked Questions</h2>
          ...
          {disease.conclusion && (
            <section id="conclusion" className="scroll-mt-28 bg-gradient-to-br from-green-50 to-white p-8 md:p-10 rounded-[2.5rem] border border-green-200 shadow-lg relative overflow-hidden">
              ...
              <h2 className="text-2xl font-bold text-[#1A361A] mb-4 flex items-center gap-2"><CheckCircle className="text-[#1EAD16]"/> Conclusion</h2>
