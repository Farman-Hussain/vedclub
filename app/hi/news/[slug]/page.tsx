...
async function getNews(baseSlug: string) {
  const hiSlug = `${baseSlug}-hi`;
  const query = `*[_type == "news" && (slug.current == $hiSlug || slug.current == $baseSlug) && language == 'hi'][0]{ ..., author-> }`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!newsItem) return {};
  return { title: newsItem.seoTitle || `${newsItem.title} | वेद क्लब समाचार`, description: newsItem.seoDescription };
}
...
export default async function HindiNewsPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const newsItem = await getNews(slug);
  ...
  return (
    <div className="bg-white min-h-screen pb-20 scroll-smooth">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        <Link href="/hi/news" className="inline-flex items-center text-gray-500 hover:text-[#1EAD16] mb-8 transition-colors font-medium">
          <ArrowLeft size={16} className="mr-2" /> समाचार हब पर वापस जाएं
        </Link>
        <div className="flex items-center gap-4 mb-6">
          <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1"><Briefcase size={12}/> अपडेट</span>
          {newsItem.lastDate && <span className="text-red-500 font-bold text-sm bg-red-50 px-3 py-1 rounded-full">अंतिम तिथि: {new Date(newsItem.lastDate).toLocaleDateString()}</span>}
        </div>
        ...
        {newsItem.applyLink && (
           <a href={newsItem.applyLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#1EAD16] text-white px-8 py-4 rounded-xl font-bold mb-8 hover:bg-green-700 hover:-translate-y-1 shadow-lg transition-all">
             <LinkIcon size={18} /> आधिकारिक लिंक / आवेदन करें
           </a>
        )}
      </section>
          ...
          <div className="sticky top-28 space-y-6">
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              ...
              <ul className="space-y-3 text-gray-600 font-medium text-sm">
                ...
                {newsItem.faqs && <li><a href="#faqs" className="hover:text-[#1EAD16] transition flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span> पूछे जाने वाले प्रश्न</a></li>}
                {newsItem.references && <li><a href="#references" className="hover:text-[#1EAD16] transition flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span> संदर्भ</a></li>}
              </ul>
            </div>
        ...
        <div className="md:w-3/4 space-y-16">
          ...
          {newsItem.author && (
            <section className="bg-white p-8 md:p-10 rounded-3xl shadow-lg border border-green-100 mt-16 hover:-translate-y-1 transition duration-300">
              <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">समीक्षक</h3>
              <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {newsItem.author.consultationLink && <a href={newsItem.author.consultationLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-[#1A361A] hover:bg-[#1EAD16] text-white px-5 py-2.5 rounded-full text-sm font-bold transition shadow-md"><Calendar size={16} /> परामर्श (Consult)</a>}
                    {newsItem.author.call && <a href={`tel:${newsItem.author.call}`} className="flex items-center gap-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 px-4 py-2 rounded-full text-sm font-medium text-gray-700 transition hover:-translate-y-0.5"><Phone size={16} className="text-blue-600" /> कॉल करें</a>}
                    {newsItem.author.whatsapp && <a href={`https://wa.me/${newsItem.author.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-green-50 hover:bg-green-100 text-green-700 px-4 py-2.5 rounded-full text-sm font-bold transition"><MessageCircle size={16} /> व्हाट्सएप</a>}
                  </div>
                </div>
              </div>
            </section>
