...
async function getAllTreatments() {
  const query = `*[_type == "treatment" && (!defined(language) || language == 'en')] | order(title asc) {
      ...
      <section className="bg-[#1A361A] text-white pt-20 pb-36 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10 animate-fade-in">
          <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md text-blue-200 text-sm font-bold mb-6 border border-white/20">
            <Activity size={16} /> Clinical Procedures
          </div>
          <h1 className="text-5xl md:text-7xl font-extrabold mb-6 tracking-tight">Ayurvedic Treatments</h1>
          <p className="text-xl text-blue-50 max-w-2xl mx-auto leading-relaxed">
            Detailed guides, benefits, and protocols for Panchakarma, Shirodhara, and other classical procedures.
          </p>
        </div>
      </section>

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-24 relative z-20">
        <SearchGrid items={treatments} type="Treatments" basePath="/treatments" />
      </section>
    </div>
  );
}