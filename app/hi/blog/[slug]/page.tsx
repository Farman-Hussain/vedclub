...
// 🔥 FETCHES HINDI BLOG POST
async function getArticle(baseSlug: string) {
  const hiSlug = `${baseSlug}-hi`;
  const query = `*[_type == "blog" && (slug.current == $hiSlug || slug.current == $baseSlug) && language == 'hi' && category == 'blog'][0]{ ..., author-> }`;
...
export default async function HindiBlogPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = await getArticle(slug);
  ...
  return (
    <div className="bg-white min-h-screen pb-20 scroll-smooth">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        <Link href="/hi/blog" className="inline-flex items-center text-gray-500 hover:text-[#1EAD16] mb-8 transition-colors font-medium">
          <ArrowLeft size={16} className="mr-2" /> ब्लॉग पर वापस जाएं
        </Link>
        <div className="flex items-center gap-4 mb-6">
          <span className="bg-green-100 text-[#1EAD16] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">ब्लॉग</span>
          ...
          <div className="sticky top-28 space-y-6">
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              ...
              <ul className="space-y-3 text-gray-600 font-medium text-sm">
                ...
                {article.faqs && <li><a href="#faqs" className="hover:text-[#1EAD16] transition flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span> अक्सर पूछे जाने वाले प्रश्न</a></li>}
