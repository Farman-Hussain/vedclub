...
async function getHerb(slug: string) {
  const query = `*[_type == "herb" && slug.current == $slug][0]{ ..., author-> }`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!herb) return {};
  return { title: herb.seoTitle || `${herb.title} | Ved Club`, description: herb.seoDescription || "Discover the ancient Ayurvedic properties, benefits, and uses of this powerful herb." };
}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-in">
            ...
            <div className="mb-8"><PortableText value={herb.shortDescription} components={ptComponents} /></div>

            {/* DYNAMIC E-COMMERCE BUTTONS */}
            <div className="mt-8">
              {(herb.storeLink || herb.amazonLink || herb.blinkitLink) ? (
                <div className="flex flex-wrap gap-4">
                  {herb.storeLink && <a href={herb.storeLink} target="_blank" rel="noreferrer" className="bg-[#1EAD16] text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-green-700 transition-all hover:-translate-y-1 hover:shadow-xl shadow-md"><ShoppingBag size={18} /> Buy Pure {herb.title}</a>}
                  {herb.amazonLink && <a href={herb.amazonLink} target="_blank" rel="noreferrer" className="bg-gray-900 text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-black transition-all hover:-translate-y-1 hover:shadow-xl shadow-md"><ShoppingCart size={18} /> Amazon</a>}
                  {herb.blinkitLink && <a href={herb.blinkitLink} target="_blank" rel="noreferrer" className="bg-[#F8CB46] text-black px-6 py-3 rounded-full font-bold flex items-center gap-2 hover:bg-yellow-400 transition-all hover:-translate-y-1 hover:shadow-xl shadow-md"><Zap size={18} /> Blinkit</a>}
                </div>
              ) : (
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between max-w-md hover:shadow-md transition">
                  <div><p className="text-sm text-gray-500">Pure {herb.title} Root Extract</p><p className="text-[#1A361A] font-bold text-lg">Coming Soon</p></div>
                  <button className="bg-gray-100 text-gray-400 px-6 py-2 rounded-full font-medium flex items-center gap-2 cursor-not-allowed"><ShoppingBag size={18} /> Notify Me</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="relative w-full aspect-square max-w-md mx-auto lg:ml-auto group p-4">
             {herb.images && herb.images.length > 0 ? (
                ...
                <div className="w-full h-full bg-green-900 rounded-[2rem] shadow-2xl relative flex items-center justify-center transition-all duration-700 group-hover:rotate-y-6 group-hover:rotate-x-6 group-hover:scale-105"><div className="text-green-100/20 text-9xl">🌿</div></div>
             )}
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 flex flex-col md:flex-row gap-12">
        ...
        <div className="md:w-1/4 hidden md:block">
          <div className="sticky top-28 space-y-6">
            ...
            {/* CLEANED UP POPUP SIDEBAR */}
            <DoctorSidebar author={herb.author} layout="vertical" />
          </div>
        </div>
        ...
        <div className="md:w-3/4 space-y-16">
              ...
              <div className="space-y-6">
                {herb.benefits.map((benefit: any, index: number) => (
                  <div key={index} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition duration-300">
                    <h3 className="text-xl font-bold text-[#1A361A] mb-4 flex items-center gap-3"><span className="bg-green-100 text-[#1EAD16] w-8 h-8 flex items-center justify-center rounded-full text-sm shrink-0">{index + 1}</span>{benefit.heading}</h3>
                    <div className="pl-11"><PortableText value={benefit.description} components={ptComponents} /></div>
                  </div>
                ))}
              </div>
            </section>
          ...
          {herb.properties && (
            <section id="properties" className="scroll-mt-28">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition duration-300">
                <div className="grid grid-cols-3 bg-green-50 border-b border-green-100 p-4 font-bold text-[#1A361A] text-sm md:text-base"><div>Property</div><div>Ayurvedic Term</div><div>Meaning</div></div>
                {[
                  ...
                  <div key={i} className={`grid grid-cols-3 p-4 border-b border-gray-100 text-sm md:text-base transition ${row.highlight ? 'bg-green-50/50 font-medium' : 'hover:bg-gray-50'}`}>
                    <div className={row.highlight ? "font-bold text-[#1EAD16]" : "font-medium text-gray-700"}>{row.name}</div><div className="text-gray-800">{row.term}</div><div className="text-gray-600">{row.meaning}</div>
                  </div>
                ))}
              </div>
            </section>
          ...
          {herb.howToConsume && (
            <section id="consume" className="scroll-mt-28">
              ...
              <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg transition duration-300"><PortableText value={herb.howToConsume} components={ptComponents} /></div>
            </section>
          )}

          {herb.additionalContent && (
            <section id="additional" className="scroll-mt-28">
              <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm hover:shadow-lg transition duration-300"><PortableText value={herb.additionalContent} components={ptComponents} /></div>
            </section>
          )}
          ...
          {herb.references && (
            <section id="references" className="scroll-mt-28">
              ...
              <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200 text-sm text-gray-600"><PortableText value={herb.references} components={ptComponents} /></div>
            </section>
          )}

          {/* LARGE BOTTOM AUTHOR SECTION */}
          {herb.author && (
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                <div className="shrink-0 group">
                  {herb.author.image ? (
                    ...
                    <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-green-100 text-[#1EAD16] flex items-center justify-center font-bold text-4xl shadow-sm group-hover:scale-105 transition">{getInitials(herb.author.name)}</div>
                  )}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {herb.author.consultationLink && <a href={herb.author.consultationLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-[#1A361A] hover:bg-[#1EAD16] border border-transparent px-5 py-2.5 rounded-full text-sm font-bold text-white transition hover:-translate-y-0.5 shadow-md"><Calendar size={16} /> Book Consult</a>}
                    {herb.author.call && <a href={`tel:${herb.author.call}`} className="flex items-center gap-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 px-4 py-2 rounded-full text-sm font-medium text-gray-700 transition hover:-translate-y-0.5"><Phone size={16} className="text-blue-600" /> Call Doctor</a>}
                    {herb.author.whatsapp && <a href={`https://wa.me/${herb.author.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-green-50 hover:bg-green-100 border border-green-200 px-4 py-2 rounded-full text-sm font-medium text-gray-700 transition hover:-translate-y-0.5"><MessageCircle size={16} className="text-green-600" /> WhatsApp</a>}
                    {herb.author.instagram && <a href={herb.author.instagram} target="_blank" rel="noreferrer" className="p-2 bg-pink-50 hover:bg-pink-100 rounded-full text-pink-600 transition hover:scale-110"><Camera size={20}/></a>}
                    {herb.author.facebook && <a href={herb.author.facebook} target="_blank" rel="noreferrer" className="p-2 bg-blue-50 hover:bg-blue-100 rounded-full text-blue-600 transition hover:scale-110"><Globe size={20}/></a>}
                    {herb.author.youtube && <a href={herb.author.youtube} target="_blank" rel="noreferrer" className="p-2 bg-red-50 hover:bg-red-100 rounded-full text-red-600 transition hover:scale-110"><PlayCircle size={20}/></a>}
                  </div>
                </div>
              </div>
            </section>
