...
async function getArticle(slug: string) {
  // 🔥 FIX 1: ADDED category == "tip" TO THE QUERY
  const query = `*[_type == "blog" && slug.current == $slug && category == "blog"][0]{ ..., author-> }`;
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!article) return {};
  return { title: article.seoTitle || `${article.title} | Ved Club`, description: article.seoDescription };
}
...
export default async function HealthTipPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = await getArticle(slug);
  ...
  // 🔥 FIX 2: EXTRACT HEADINGS FOR TOC
  ...
  return (
    <div className="bg-white min-h-screen pb-20 scroll-smooth">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
        <Link href="/health-tips" className="inline-flex items-center text-gray-500 hover:text-[#1EAD16] mb-8 transition-colors font-medium">
          <ArrowLeft size={16} className="mr-2" /> Back to Tips
        </Link>
        <div className="flex items-center gap-4 mb-6">
          <span className="bg-green-100 text-[#1EAD16] px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Health Tip</span>
          ...
          <div className="sticky top-28 space-y-6">
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              ...
              <ul className="space-y-3 text-gray-600 font-medium text-sm">
                ...
                {article.faqs && <li><a href="#faqs" className="hover:text-[#1EAD16] transition flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span> FAQs</a></li>}
                {article.references && <li><a href="#references" className="hover:text-[#1EAD16] transition flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-gray-300"></span> References</a></li>}
              </ul>
            </div>
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {article.author.consultationLink && <a href={article.author.consultationLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-[#1A361A] hover:bg-[#1EAD16] text-white px-5 py-2.5 rounded-full text-sm font-bold transition shadow-md"><Calendar size={16} /> Book Consult</a>}
                    {article.author.whatsapp && <a href={`https://wa.me/${article.author.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-green-50 hover:bg-green-100 text-green-700 px-4 py-2.5 rounded-full text-sm font-bold transition"><MessageCircle size={16} /> WhatsApp</a>}
                  </div>
                </div>
              </div>
            </section>
